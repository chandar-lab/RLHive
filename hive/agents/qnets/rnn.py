...
class MLPRNNNetwork(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.input_mlp = MLPNetwork(in_dim, mlp_layers, noisy=noisy, std_init=std_init)

        # RNN Layers
        conv_output_size = calculate_output_dim(self.input_mlp, in_dim)
        self.rnn = sequence_fn(
            rnn_input_size=np.prod(conv_output_size),
        )
    ...
    def forward(self, x, hidden_state=None):
        ...
        if len(x.shape) == 2:
            ...
            B, L, C = x.size()
        x = self.input_mlp(x.reshape((B * L, -1)))
        x = x.view(B, L, -1)
        if hidden_state is None:
            hidden_state = self.init_hidden(B)
        x = torch.flatten(x, start_dim=2, end_dim=-1)  # (B, L, -1)
