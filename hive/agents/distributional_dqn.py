...
class DistributionalDQNAgent(Agent):
    ...
    def __init__(
        ...
        """
        super().__init__(obs_dim=obs_dim, act_dim=act_dim, id=f"distributional_dqn_agent_{id}")
        ...
        self._delta_z = (self._vmax - self._vmin) / (self._num_atoms - 1)
        z_points = np.linspace(self._vmin, self._vmax, self._num_atoms).astype(np.float32)
        self._zpoints = torch.from_numpy(z_points).unsqueeze(0).to(device)
