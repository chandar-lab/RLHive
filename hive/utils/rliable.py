...
def load_and_read_rewards(agent_name,env_names,path,min_env_score,max_env_score,normalization):
  score_dict = {}
  for env_index,env_name in enumerate(env_names):
    ...
    score_ = []
    for seed in seed_s:
      job_files = [i for i in os.listdir(os.path.join(path_temp,seed)) if i[0] != "."]
      for job_file in job_files:
        logger_files = [i for i in os.listdir(os.path.join(path_temp,seed,job_file,'logger')) if i[0] != "."]
        for logger_file in logger_files:
          pkl_file = open(os.path.join(path_temp,seed,job_file,'logger',logger_file,'log_data.p'),'rb')
  ...
  median, mean = metrics.aggregate_median(score_matrix), metrics.aggregate_mean(score_matrix)
  return score_matrix

def get_rliable_parameters(num_runs_aggregates,
                           path,
                           output_path,
                           test_agent,
                           normalization=True,
  ...
  agent_names = [i for i in os.listdir(os.path.join(path,env_names[0])) if i[0] != "."]
  ...
  for agent_name in agent_names:
    score_data_dict[agent_name] = load_and_read_rewards(agent_name,env_names,path,min_env_score,max_env_score,normalization)
...
def main():
    ...
    parser.add_argument("-min_env_score", "--min_env_score",default=[-1.0,-1.0,0.0],nargs="+")
    parser.add_argument("-max_env_score", "--max_env_score",default=[0.0,1.0,1.0],nargs="+")
    ...
    get_rliable_parameters(
        ...
        args.test_agent,
        args.normalization,
        args.min_env_score,args.max_env_score)
