...
class Runner:
    ...
    def __init__(
        ...
        self._transition_info = TransitionInfo([self._agent])
    ...
    def train_mode(self, training):
        ...
        """
        self._agent.train() if training else self._agent.eval()
    ...
    def run_end_step(self, episode_metrics):
        ...
        """
        info = self._transition_info.get_info(self._agent, done=True)
        self._agent.update(info)
        episode_metrics[self._agent.id]["reward"] += info["reward"]
        episode_metrics[self._agent.id]["episode_length"] += 1
        episode_metrics["full_episode_length"] += 1
