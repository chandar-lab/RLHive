...
def test_run_episode(initial_runner, max_steps_per_episode):
    """
    ...
    assert (
        episode_metrics["full_episode_length"]
        == len(multi_agent_runner._agents) * max_steps_per_episode
    )
    for agent in multi_agent_runner._agents:
...
def test_resume(initial_runner, max_steps_per_episode):
    ...
    resumed_multi_agent_runner._train_step_schedule = (
        resumed_multi_agent_runner._experiment_manager.experiment_state[
            "train_step_schedule"
        ]
    )
    resumed_multi_agent_runner._train_episode_schedule = (
        resumed_multi_agent_runner._experiment_manager.experiment_state[
            "train_episode_schedule"
        ]
    )
    resumed_multi_agent_runner._test_schedule = (
        resumed_multi_agent_runner._experiment_manager.experiment_state["test_schedule"]
    )
