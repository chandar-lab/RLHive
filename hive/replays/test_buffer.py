...
@pytest.fixture()
def test_add_to_buffer(initialize_buffer):
    """
        test adding one transition to the buffer
    """
    buffer, environment, rng = initialize_buffer
    observation, _ = environment.reset()
    action = rng.integers(environment._env_spec.act_dim)
    next_observation, reward, done, turn, info = environment.step(action)
    buffer.add((observation,
...
def test_sample_from_buffer(batch_size, initialize_buffer):
    """
    test sampling a batch from the buffer
    """
    buffer, environment, _ = initialize_buffer
...
def test_saving_buffer(tmpdir, initialize_buffer):
    """
        test sampling a batch from the buffer
    """
    buffer, environment, _ = initialize_buffer
...
@pytest.mark.parametrize("batch_size", [32])
def test_loading_buffer(tmpdir, batch_size, initialize_buffer):
    """
        test sampling a batch from the buffer
    """
    buffer, environment, rng = initialize_buffer
